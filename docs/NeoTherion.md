...
```json
{
  "rules": [
    {
      "triggers": ["hello", "hi"],
      "responses": [{"weight": 1, "text": "# Access Log\nHi! I'm **NeoTherion**. What's your name?"}],
      "stateWeights": {"Positive": 1},
      "requiredStates": []
    },
    {
      "triggers": ["[user]"],
      "responses": [{"weight": 1, "text": "### Profile Established\nNice to meet you, **(User)**!"}],
      "stateWeights": {"Neutral": 1},
      "requiredStates": ["Positive"]
    }
  ]
...
```json
{
  "rules": [
    {
      "triggers": ["pizza"],
      "responses": [{"weight": 1, "text": "I dream of pizza! Do you like pizza?"}],
      "stateWeights": {"Positive": 1}
    },
    {
      "triggers": ["+yes"],
      "responses": [{"weight": 1, "text": "Me too! **(User|Friend)**."}],
    ...
    },
    {
      ...
      "responses": [{"weight": 1, "text": "> **SYSTEM**: Pizza affinity saved."}],
      "stateWeights": {"[Maintain State]": 1}
    }
  ]
}
...
```json
{
  "rules": [
    {
      "triggers": ["matrix theme"],
      "responses": [{
        "weight": 1,
        "text": "# Protocol Green\n<script>(async () => { const root = document.documentElement; const colors = { '--bg': '#000800', '--panel': '#001a00', '--accent': '#00ff41', '--text': '#00ff41', '--border': '#003b00', '--beast-gradient': 'linear-gradient(135deg, #00ff41 0%, #003b00 100%)' }; Object.entries(colors).forEach(([k, v]) => root.style.setProperty(k, v)); })();</script>"
      }]
    }
  ]
}
  ...
  "rules": [
    {
      ...
      "responses": [{
        "weight": 1,
        "text": "# Combat Log\n<script>(async () => { const roll = Math.floor(Math.random() * 20) + 1; document.querySelector('.bubble.bot:last-child').innerHTML += `<br>Result: **${roll}**`; })();</script>"
      }]
    }
  ]
}
  ...
  "rules": [
    {
      ...
      "responses": [{
        "weight": 1,
        "text": "### Console\n<div style='display:flex; gap:8px;'><button style='background:var(--accent); color:white; border:none; padding:8px;' onclick=\"document.getElementById('user-input').value='roll d20'; window.handleSend();\">ROLL</button></div>"
      }]
    }
  ]
}
...
```json
{
  "rules": [
    {
      "triggers": ["check status"],
      "responses": [{
        "weight": 1,
        "text": "Analyzing... <script>(async () => { const h = new Date().getHours(); const s = (h < 6 || h > 20) ? 'Negative' : 'Positive'; await NeoTherion.setStatus(s); })();</script>"
      }]
    }
  ]
}
  ...
  "rules": [
    {
      ...
      "responses": [{
        "weight": 1,
        "text": "### Fetching...\n<script>(async () => { const r = await fetch('https://api.open-meteo.com/v1/forecast?latitude=40.71&longitude=-74.01&current_weather=true'); const d = await r.json(); document.querySelector('.bubble.bot:last-child').innerHTML += `| Metric | Value |\\n| :--- | :--- |\\n| Temp | ${d.current_weather.temperature}°C |`; })();</script>"
      }]
    }
  ]
}
...
**Test**: Type phrases like "tell me what is real" or "what is the essence of being".
**Expected Output**: The bot matches via `VECTOR` logic even without keyword overlap. Confidence badge shows `VECTOR XX%`.

**Calibration Instructions**:

1. **Import** the rule above
2. **Enable** Allow Response Scripts (for visual feedback)
3. **Don't type exact triggers**. Instead use similar phrases:
   - "tell me what is real"
   - "I want to talk about existence"
   - "what's the meaning of everything"

4. **Open Debug Tab** → Find "Vector Threshold" slider
5. **Test Different Thresholds**:
   - **0.90** (Strict): Likely fails, enters Dream Mode
   - **0.75** (Default): Should match successfully
   - **0.65** (Lenient): Matches easily, may have false positives

**Expected Behavior**:
- Confidence badge shows "VECTOR XX%"
- Visual feedback shows "SEMANTIC MATCH"
- Debug panel shows vector similarity score

**Troubleshooting**: If you see "NEURAL SYNCED" chip at top, vectors are ready. If not, wait 5-10 seconds after import.

```json
{
  "rules": [
    {
      "triggers": ["philosophical inquiry", "nature of reality", "meaning of life"],
      "responses": [{"weight": 1, "text": "> You seek understanding of **existence**."}]
    }
  ]
}
```
  ...
  "rules": [
    {
      ...
      "responses": [{
        "weight": 1,
        "text": "### Syncing...\n<script>(async () => { const d = new Dexie('NeoTherion'); await d.open(); const h = await d.table('history').where('role').equals('user').reverse().limit(3).toArray(); const q = h[2].text; const a = h[1].text; await d.table('rules').add({ triggers: [q], responses: [{weight:1, text:a}], stateWeights: {'[Maintain State]':1}, requiredStates:[] }); })();</script>"
      }]
    }
  ]
}
  ...
  "rules": [
    {
      ...
      "responses": [{
        "weight": 1,
        "text": "### WARNING\n<script>(async () => { if(confirm('Purge all data?')) { await Dexie.delete('NeoTherion'); location.reload(); } })();</script>"
      }]
    }
  ]
}
...
```json
{
  "rules": [
    {
      "triggers": ["markdown test"],
      "responses": [{ "weight": 1, "text": "# UI Test\n| Component | Status |\n| :--- | :--- |\n| Vector Thread | OK |\n```js\nconsole.log('Beast Awake');\n```\n- System Stable" }]
    }
  ]
}
```
